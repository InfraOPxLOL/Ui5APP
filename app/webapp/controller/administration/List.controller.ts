...
} from "../../service/administration/AdministrationService";
import AdministrationFormatter from "../../formatter/administration/AdministrationFormatter";
import AdministrationModel from "../../model/administration/AdministrationModel";
import { administrationTableConfig } from "../../config/administration/columns";

/**
 * List controller for the Administration module.
 *
 * Applies the declarative table configuration to the shared {@link ConfigurableTable}, binds it to
 * the module view model, and loads data through {@link AdministrationService}. It holds no business
 * logic — data shaping lives in the service, formatting in the formatter/core layer.
 *
 * @namespace com.middlewareops.integrationportal.controller.administration
 */
export default class ListController extends BaseController {
  /** Exposed for optional formatter bindings in the view. */
  public readonly formatter = AdministrationFormatter;
   ...
   */
  public onInit(): void {
    this.setModel(new AdministrationModel(), "view");
    const table = this.byId("table") as unknown as ConfigurableTable;
    table.applyConfiguration(administrationTableConfig);
  ...
  public onExport(): void {
    const items = (this.getModel("view") as JSONModel).getProperty(
      "/items",
    ) as AdministrationItem[];
    const columns = administrationTableConfig.columns.map((column) => ({
      property: column.property as keyof AdministrationItem,
      label: column.property,
    }));
    ExportHelper.exportCsv(items, columns, "administration");
  }
}
